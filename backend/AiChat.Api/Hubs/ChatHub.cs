...
public sealed class ChatHub(IAiChatService aiChatService, IChatService chatService, ILogger<ChatHub> logger) : Hub
{
    public async Task SendMessage(string sessionId, string text)
    {
        ...
        try
        {
            ...
            var message = await chatService.AddMessageAsync(userId, parsedSessionId, MessageRole.User, text, Context.ConnectionAborted);
            ...
            var assistantMessage = await chatService.AddMessageAsync(
                userId, parsedSessionId, MessageRole.Assistant, assistantContent.ToString(), Context.ConnectionAborted);
